...
def plotar_grafico_calibracao_p(df_calibracao: pd.DataFrame, info_limites_p: dict, caminho_saida_grafico: str) -> bool:
    ...
    try:
        ...
        if len(pontos_fora) > 0:
            print("Aviso de Calibração P: Pontos encontrados fora dos limites de controle.")

        fig, ax = plt.subplots(figsize=(12, 7))
        
        ax.plot(df['lote'], df['p'], marker='o', linestyle='-', color='b', label='Proporção (p) do Lote')
        ax.axhline(p_barra, color='g', linestyle='-', label=f'Linha Média (p-barra)={p_barra:.4f}')
        ax.step(df['lote'], df['LSC_p'], color='r', linestyle='--', where='mid', label='LSC (Variável)')
        ax.step(df['lote'], df['LIC_p'], color='r', linestyle='--', where='mid', label='LIC (Variável)')

        if len(pontos_fora) > 0:
            ax.scatter(pontos_fora['lote'], pontos_fora['p'], s=100, facecolors='none', edgecolors='r', label='Fora de Controle')
...
def plotar_grafico_calibracao_u(df_calibracao: pd.DataFrame, info_limites_u: dict, caminho_saida_grafico: str) -> bool:
    ...
    try:
        ...
        if len(pontos_fora) > 0:
            print("Aviso de Calibração U: Pontos encontrados fora dos limites de controle.")

        fig, ax = plt.subplots(figsize=(12, 7))
        
        ax.plot(df['amostra'], df['u'], marker='o', linestyle='-', color='b', label='Taxa de Defeitos (u)')
        ax.axhline(u_barra, color='g', linestyle='-', label=f'Linha Média (u-barra)={u_barra:.4f}')
        ax.step(df['amostra'], df['LSC_u'], color='r', linestyle='--', where='mid', label='LSC (Variável)')
        ax.step(df['amostra'], df['LIC_u'], color='r', linestyle='--', where='mid', label='LIC (Variável)')

        if len(pontos_fora) > 0:
            ax.scatter(pontos_fora['amostra'], pontos_fora['u'], s=100, facecolors='none', edgecolors='r', label='Fora de Controle')
