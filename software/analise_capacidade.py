...
def calcular_sigma_estimado(R_barra: float, n_amostra: int, constantes_db: dict) -> float | None:
...
def calcular_probabilidade_lt(mu: float, sigma: float, LSE: float, LIE: float, Cpk: float) -> dict:
...
def executar_analise_completa(info_limites_xr: dict, constantes_db: dict, especificacoes: dict) -> dict | None:
    ...
    try:
        ...
        info_prob_lt = calcular_probabilidade_lt(mu, sigma, LSE, LIE, info_capacidade['Cpk'])
        ...
        if valor_arbitrario is not None:
            try:
                ...
                print(f"Calculando probabilidade arbitrária para P(X > {valor_arb_float})...")
                ...
                print(f"Resultado: P(X > {valor_arb_float}) = {prob_acima:.6f} ({prob_acima * 1_000_000:.2f} PPM)")
            
            except ValueError:
                print(f"AVISO: 'valor_prob_arbitrario' ({valor_arbitrario}) não é um número válido. Cálculo pulado.")
            except Exception as e:
                print(f"AVISO: Erro ao calcular probabilidade arbitrária: {e}. Cálculo pulado.")
